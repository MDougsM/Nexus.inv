...
export default function ModalFicha({ modalFicha, setModalFicha, categorias, usuarioAtual, carregarDados }) {
  ...
  const toggleProtecaoC2 = async () => {
    ...
    try {
      ...
      await api.put(`/api/inventario/ficha/editar/${ativo.patrimonio}`, payload);
      ...
      <div className="w-full max-w-5xl max-h-[95vh] rounded-3xl shadow-2xl border overflow-hidden flex flex-col print:border-none print:shadow-none print:max-h-none print:h-auto" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-light)' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 custom-scrollbar print:p-4">
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Segurança e Especificações Omitidas por brevidade, mas você pode usar o bloco antigo de HW aqui */}
             {ativo.dados_dinamicos && camposPermitidos.length > 0 && (
              <div className="p-6 rounded-3xl border shadow-sm print:border-gray-300" style={{ backgroundColor: 'var(--bg-input)', borderColor: 'var(--border-light)' }}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setModalAvancado(true)} className="bg-blue-600 text-white text-[9px] px-3 py-1.5 rounded-lg uppercase font-bold hover:bg-blue-700 transition-all print:hidden">Ver Avançado 🔎</button>
                </div>
                <div className="space-y-4">
                    ...
                    return (
                      <div key={campo}>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <p className="font-black text-sm flex-1" style={{ color: 'var(--text-main)' }}>{dadosDin[campo]}</p>
          ...
          {/* Histórico */}
          <div className="lg:col-span-7 p-8 rounded-3xl border shadow-sm print:border-none print:shadow-none print:p-0" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-light)' }}>
