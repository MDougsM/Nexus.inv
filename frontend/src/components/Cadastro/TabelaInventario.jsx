        ...
        <thead style={{ backgroundColor: 'var(--bg-input)', borderBottom: '1px solid var(--border-light)' }}>
          <tr>
            <th className="p-4 w-10">
              <input type="checkbox" className="w-4 h-4 rounded cursor-pointer" checked={selecionados.length === ativosPaginaAtual.length && ativosPaginaAtual.length > 0} onChange={(e) => handleSelectAll({ ...e, target: { ...e.target, checked: e.target.checked, list: ativosPaginaAtual }})} />
            </th>
            ...
            <th className="p-4 text-[10px] font-black uppercase tracking-widest opacity-60 hidden sm:table-cell" style={{ color: 'var(--text-main)' }}>Status Físico</th>
            <th className="p-4 text-[10px] font-black uppercase tracking-widest opacity-60" style={{ color: 'var(--text-main)' }}>Hardware</th>
            <th className="p-4 text-[10px] font-black uppercase tracking-widest opacity-60 hidden md:table-cell" style={{ color: 'var(--text-main)' }}>Unidade / Localização</th>
                ...
                <React.Fragment key={ativo.id}>
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="group relative flex items-center justify-center ml-1">
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-max bg-gray-900 text-white text-[10px] px-2 py-1.5 rounded-lg z-50">
                            {online ? <span className="font-bold text-emerald-400">🟢 Agente Online</span> : <div className="flex flex-col items-center"><span className="font-bold text-gray-300 mb-0.5">⚪ Agente Offline</span><span className="text-[9px] text-gray-400">Visto em: {formatarDataUltimoAcesso(ativo.ultima_comunicacao)}</span></div>}
                          </div>
                        </div>
                    ...
                    <td className="p-4 hidden md:table-cell">
                      <div className="font-black text-xs uppercase text-blue-600">{ativo.unidade ? ativo.unidade.nome : (ativo.secretaria || 'Não Alocado')}</div>
                      <div className="text-[10px] font-bold opacity-60 mt-0.5" style={{color:'var(--text-main)'}}>{ativo.unidade ? ativo.unidade.tipo : (ativo.setor || '-')}</div>
                    </td>
